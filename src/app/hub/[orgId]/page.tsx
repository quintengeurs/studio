...
export default function HubPage({ params }: { params: { orgId: string } }) {
  ...
  const selectedTask = useMemo(() => {
    const allPossible = [...tasks, ...myCompletedTasks];
    return allPossible.find(t => t.id === selectedTaskId) || null;
  }, [tasks, myCompletedTasks, selectedTaskId]);
          ...
          <TabsContent value="volunteers">
            <div className="space-y-6">
              <div className="flex items-center justify-between">
                ...
                <Badge variant="outline" className="font-bold">{allVolunteers.filter(v => v.status === 'active').length} Members</Badge>
              </div>

              {volunteersLoading ? (
                <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
                  {[1, 2, 3, 4, 5, 6].map((i) => (
                    <div key={i} className="h-48 rounded-3xl border-2 bg-muted/20 animate-pulse flex flex-col items-center justify-center p-6 space-y-4">
                ...
                <div className="flex flex-col items-center justify-center py-20 border-2 border-dashed rounded-3xl bg-muted/20 opacity-60">
                  <Users className="h-12 w-12 mb-4 text-orange-500 opacity-20" />
                ...
                <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
                  {allVolunteers.filter(v => v.status === 'active').map(v => (
                    <Card key={v.id} className="p-6 border-2 border-orange-500/10 hover:border-orange-500/20 transition-all flex flex-col items-center text-center rounded-3xl">
                      <Avatar className="h-20 w-20 mb-4 border-4 border-white shadow-lg">
                        <AvatarFallback className="bg-gradient-to-br from-orange-500 to-amber-400 text-white font-bold text-xl">{v.email?.charAt(0).toUpperCase()}</AvatarFallback>
                      </Avatar>
                      <h4 className="font-bold text-foreground text-lg mb-1 truncate w-full">{v.email}</h4>
                      <div className="flex items-center gap-2 mb-4">
                        <Badge variant="secondary" className="bg-orange-50 text-orange-600 border-orange-100 text-[10px] font-bold">⭐ {v.totalPoints || 0} PTS</Badge>
                        <Badge variant="secondary" className="bg-green-50 text-green-600 border-green-100 text-[10px] font-bold">✓ {v.completedTasksCount || 0} TASKS</Badge>
                      </div>
                      <div className="w-full pt-4 border-t border-dashed border-orange-200 mt-auto">
                        <p className="text-[10px] text-muted-foreground uppercase font-bold tracking-widest mb-1">Last Active</p>
                        <p className="text-xs font-medium">{v.lastActive ? format(new Date(v.lastActive), 'MMM d, yyyy') : 'Recently'}</p>
                      </div>
                    </Card>
                  ))}
                </div>
          ...
          <TabsContent value="tasks_mgmt">
            <div className="space-y-6">
              <div className="flex items-center justify-between">
                ...
                <Button className="bg-orange-500 hover:bg-orange-600 font-bold rounded-xl" onClick={() => setIsCreateTaskModalOpen(true)}>
              ...
              <div className="grid gap-6">
                <div className="space-y-4">
                  <h4 className="text-sm font-black uppercase tracking-widest text-muted-foreground flex items-center gap-2">
                    ...
                    Active & Upcoming ({staffActiveTasks.length})
                  </h4>
                  <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                    {staffActiveTasks.map(task => (
                      <Card key={task.id} className="p-5 border-2 border-orange-500/10 hover:border-orange-500/30 transition-all group rounded-3xl relative">
                        <div className="absolute top-4 right-4">
                           <Button variant="ghost" size="icon" className="h-8 w-8 rounded-full hover:bg-orange-100 text-orange-600" onClick={() => { setEditingTaskData(task); setIsEditTaskModalOpen(true); }}>
                              <Pencil className="h-4 w-4" />
                           </Button>
                        </div>
                        <div className="mb-3">
                          <Badge variant="outline" className="text-[10px] font-bold text-orange-600 border-orange-200 uppercase tracking-widest">{task.park}</Badge>
                        </div>
                        <h5 className="font-bold text-lg mb-2 pr-8">{task.title}</h5>
                        <div className="space-y-2 mb-4">
                          ...
                          <div className="flex items-center gap-2 text-xs text-muted-foreground">
                            ...
                            <span>{task.doingByVolunteers?.length || 0} Active / {task.maxVolunteers || '∞'} Max</span>
                          </div>
                        </div>
                        <div className="pt-4 border-t border-orange-50">
                           <p className="text-[10px] font-bold uppercase text-orange-600 mb-1">Reward</p>
                           <p className="text-xs font-medium italic">{task.rewardDescription || 'No reward set'}</p>
                        </div>
                      </Card>
                    ))}
                  </div>
                ...
                <div className="space-y-4 pt-6">
                  <h4 className="text-sm font-black uppercase tracking-widest text-muted-foreground flex items-center gap-2">
                    ...
                    Archive / Historical ({staffArchivedTasks.length})
                  </h4>
                  <div className="grid gap-3">
                    {staffArchivedTasks.map(task => (
                      <div key={task.id} className="p-3 rounded-2xl bg-muted/20 border border-muted flex items-center justify-between opacity-60 hover:opacity-100 transition-opacity">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            ...
                            <p className="text-[10px] text-muted-foreground uppercase font-bold">{task.park} • {task.status}</p>
                          </div>
                        </div>
                        <Button variant="ghost" size="sm" className="font-bold text-xs" onClick={() => { setEditingTaskData(task); setIsEditTaskModalOpen(true); }}>Edit</Button>
                      </div>
                    ))}
                  </div>
                </div>
          ...
          <TabsContent value="news_mgmt">
            <div className="space-y-6">
              <div className="flex items-center justify-between">
                ...
                <Button className="bg-orange-500 hover:bg-orange-600 font-bold rounded-xl" onClick={() => { setEditingNewsItem(null); setIsInfoModalOpen(true); }}>
                  <Plus className="h-4 w-4 mr-2" /> Add Hub Update
                </Button>
              </div>

              <div className="grid gap-4">
                {volunteerNews.filter(n => !n.isArchived).map(item => (
                  <Card key={item.id} className="p-5 border-2 border-orange-500/10 hover:border-orange-500/30 transition-all rounded-3xl flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                    <div className="flex items-start gap-4">
                      <div className={`h-12 w-12 rounded-2xl ${item.type === 'CTA' ? 'bg-orange-500 text-white' : 'bg-orange-100 text-orange-600'} flex items-center justify-center shrink-0`}>
                        {item.type === 'CTA' ? <HandMetal className="h-6 w-6" /> : <Megaphone className="h-6 w-6" />}
                      </div>
                      <div>
                        <div className="flex items-center gap-2 mb-1">
                          <Badge variant="outline" className="text-[9px] font-bold uppercase tracking-widest">{item.type}</Badge>
                          <span className="text-[10px] text-muted-foreground font-bold">{item.createdAt ? format(new Date(item.createdAt), 'MMM d, yyyy') : 'Recently'}</span>
                        </div>
                    ...
                    <div className="flex gap-2 w-full sm:w-auto">
                      {(item.type === 'CTA' || item.allowResponse) && (
                        <Button 
                            variant="outline" 
                            size="sm" 
                            className="font-bold text-xs flex-1 sm:flex-none border-orange-200 text-orange-600 hover:bg-orange-50"
                            onClick={() => { setSelectedItemForList(item); }}
                        >
                          <Users className="h-4 w-4 mr-2" /> {item.interestedUserIds?.length || 0} Interested
                        </Button>
                      )}
                      <Button variant="ghost" size="icon" className="h-9 w-9 rounded-full text-muted-foreground hover:text-red-500" onClick={() => handleArchiveInfo(item.id)}>
                        <Archive className="h-4 w-4" />
                      </Button>
                      <Button variant="ghost" size="icon" className="h-9 w-9 rounded-full text-muted-foreground hover:text-orange-500" onClick={() => { setEditingNewsItem(item); setIsInfoModalOpen(true); }}>
                        <Pencil className="h-4 w-4" />
                      </Button>
                    </div>
                  </Card>
